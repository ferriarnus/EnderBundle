...
public class EnderBundleClientToolTip implements ClientTooltipComponent{
	...
	public void renderImage(Font pFont, int pMouseX, int pMouseY, PoseStack pPoseStack, ItemRenderer pItemRenderer,
			int pBlitOffset, TextureManager pTextureManager) {
		for (int i1 = 0; i1 < items.size(); ++i1) {
			int j1 = pMouseX + i1 * 18 + 1;
			this.renderSlot(j1, pMouseY +1 , i1, pFont, pPoseStack, pItemRenderer, pBlitOffset, pTextureManager);
		}
	}
	
	private void renderSlot(int pX, int pY, int pItemIndex, Font pFont, PoseStack pPoseStack, ItemRenderer pItemRenderer, int pBlitOffset, TextureManager pTextureManager) {
		ItemStack itemstack = this.items.get(pItemIndex);
		this.blit(pPoseStack, pX, pY, pBlitOffset, pTextureManager, 0, 0, 18, 20);
	...
	private void blit(PoseStack pPoseStack, int pX, int pY, int pBlitOffset, TextureManager pTextureManager, float x, float y, int w, int h) {
